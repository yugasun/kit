...
export class Word32Array {
     ...
     */
    public constructor(
        words?: number[] | Word32Array | ByteArray | unknown,
        nSignificantBytes?: number,
    ) {
        if (Array.isArray(words) || !words) {
            this._words = Array.isArray(words) ? words : [];
            this._nSignificantBytes =
                typeof nSignificantBytes === 'number'
                    ? nSignificantBytes
                    : this._words.length * 4;
            return;
    ...
    public concat(w: Word32Array) {
        ...
        if (this._nSignificantBytes % 4) {
            ...
            for (let i = 0; i < N; i++) {
                ...
                this._words[(this._nSignificantBytes + i) >>> 2] |=
                    b << (24 - ((this._nSignificantBytes + i) % 4) * 8);
            }
        } else {
